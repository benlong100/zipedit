...
## 5b. Selecting deep in a document

Reported from real use: with `OA-Space` latched, selecting was fine near the
top of a 1,750-word document and painful near the bottom.

`RENDER` caches where the top visible line begins — `TOPADDR`/`TOPOK` — so it
need not count down from byte zero on every redraw. That cache was refused
whenever a selection was up, on the reasoning that the selection bounds are
logical positions and `RPOS` only comes out right if every byte is counted. So
with a selection, every keystroke walked the whole document above the
viewport, and the cost grew with depth.

By the time the cache is consulted the guard has already proved `TOPADDR` is
behind the gap, and the text before the gap is contiguous from the buffer
base — so the logical position of that address is `TOPADDR` minus the base,
with nothing to count. `SELPRIME` then decides `INSEL` on arrival, which
`SELTEST` cannot do: it only fires where `RPOS` lands exactly on a bound, and a
walk that starts partway down never lands on one.

**Measured** at line 111 of a nine-kilobyte document, ten down-arrows with a
selection up: **11.2s before, 2.9s after** — about 1.1s of lag per keypress
against 0.29s.

Why it surfaced now: the cursor-only redraw made ordinary movement
depth-independent, which left selection as the only path still paying the old
cost.

### Four tests that could not fail

Worth writing down, because each was a different way of proving nothing.

1. **Asserting the selection renders correctly.** The old code was correct,
   only slow; both versions pass.
2. **Asserting keystrokes were not dropped.** They never were. That number
   came from a measuring harness that polled until the line number held still
   for three reads — which gives up early whenever a redraw takes longer than
   a poll, and reports queued keystrokes as lost. The artefact was reported as
   a finding before it was checked.
3. **Timing it at the suite's default emulator speed.** The suite runs the
   machine flat out, where a redraw that walks the whole document still
   finishes between keystrokes. The difference disappears entirely.
4. **Timing it at 1MHz — with no selection.** `OA-Space` is a LATCH, and the
   earlier half of the same test section had left it on; pressing it again
   turned selecting off. The burst measured ordinary cursor movement, which
   takes the fast path regardless. It passed either way until `SELMODE` was
   dumped out of the machine and found to be zero.

The test now times to a known end state — the cursor having arrived ten lines
down — at 1MHz, with the latch cancelled first, and it fails with the fix
removed.

`DEEPDOC.TXT` is generated by `tools/asciifixtures.py` rather than committed,
for the same reason the line-ending fixtures are: a file whose whole purpose
is to be long enough to be slow is one a checkout could quietly shorten.
